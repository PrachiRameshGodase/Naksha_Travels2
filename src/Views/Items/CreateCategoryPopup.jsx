                    ...
                    <div className="midpopusec12x">
                        <div className="form_commonblock">
                            ...
                            <span>
                                ...
                                <input name="name" placeholder='Enter Name' autoFocus="on" value={formData.name} onChange={handleChange} autocomplete="off" />
                            </span>
                        </div>
                        <div tabIndex="0" className={`submitbuttons1  ${!formData?.name ? "disabledfield" : ""}`} onClick={() => handleSubmitCategory()}>
                            <span>
...
export const CreateSubCategoryPopup = ({ categoryData, setClickTrigger, setShowPopup, refreshCategoryListData, parent_id, subCatId }) => {
    const Navigate = useNavigate();
    const dispatch = useDispatch();
    const createCategory = useSelector(state => state?.createCategory);

    const findSubCatName = categoryData?.sub_category?.find(val => val?.id === subCatId)

    const [formData, setFormData] = useState({
        id: subCatId ? subCatId : 0,
        name: subCatId ? findSubCatName?.name : "",
    ...
    return (
        <>
            ...
            <div className="mainxpopups1" ref={popupRef} onKeyDown={handleKeyDown} tabIndex="0">
                <div className="popup-content">
                    <span className="close-button" onClick={() => setShowPopup(false)}><RxCross2 /></span>
                    {subCatId ? <h2>Update Sub-Category</h2> : <h2>New Sub-Category</h2>}
                    ...
                    <div className="midpopusec12x">
                        <div className="form_commonblock">
                            ...
                            <span>
                                ...
                                <input name="name" placeholder='Enter Sub-Category Name' autoFocus="on" value={formData.name} onChange={handleChange} autocomplete="off"/>
                            </span>
                        </div>
                        <div tabIndex="0" className={`submitbuttons1  ${!formData?.name ? "disabledfield" : ""}`} onClick={() => handleSubmitCategory()}>
                            <span>  
                                <p>
                                    {/* {categoryData?.id ? (data?.loading === true ? "Updating" : "Update") : (data?.loading === true ? "Submitting" : "Submit")} */}
                                    {subCatId ? "Update" : "Create"}
                                </p>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width={20} height={20} color={"#000000"} fill={"none"}>
                                    <path d="M20 12L4 12" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                                    <path d="M15 17C15 17 20 13.3176 20 12C20 10.6824 15 7 15 7" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
