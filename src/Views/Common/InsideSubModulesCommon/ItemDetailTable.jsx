...
import { currencySymbol, parseJSONofString, showAmountWithCurrencySymbol, showAmountWithCurrencySymbolWithPoints, showRateWithPercent } from '../../Helper/HelperFunctions'
    ...
    return (
        <>
            ...
            <div className="finalcalculateiosxl44s">
                ...
                {section === "bill" ?
                    <>
                        <span><p>Payment Made</p> <h5>{showAmountWithCurrencySymbol(itemsData?.amount_paid)}</h5></span>
                        <span><p>Amount In Excess</p> <h5>{showAmountWithCurrencySymbol(itemsData?.amt_excess)}</h5></span>
                    </> :
    ...
    return (
        <>
            ...
            <div className="finalcalculateiosxl44s">
                ...
                {showCharges && <>
                    {totalExpenseCharges?.map(val => (
                        <span><p>{val?.account?.account_name}</p> <h5>{showAmountWithCurrencySymbol(val?.amount)}</h5></span>
                    ))}
                </>}
                <span><p>Total Tax</p> <h5>{showAmountWithCurrencySymbol(calculateTotalTaxAmount())}</h5></span>


                <span><p>Total GRN Charge</p> <h5>{showAmountWithCurrencySymbol(GRNdetail?.total_grn_charges)}</h5></span>
        ...
        <>
            <table id="tablex15s56s31s1">
                <thead className="thaedaksx433">
                    <tr>
                        <th className='sfdjklsd1xs2w1'>#</th>
                        <th className='sfdjklsd1xs2w2' style={{ width: "25%" }}>Bill Number</th>
                        <th className='sfdjklsd1xs2w3' style={{ width: "10%" }}>Bill Date</th>
                        <th className='sfdjklsd1xs2w4' style={{ textAlign: "right" }}>Amount Paid</th>
                ...
                <tbody>
                    {payment?.entries?.map((val, index) => (
                        <tr className="rowsxs15aksx433" key={index}>
                            ...
                            <td className='sfdjklsd1xs2w3'>{formatDate4(val?.bill?.transaction_date)}</td>
                            <td className='sfdjklsd1xs2w4' style={{ textAlign: "right" }}>{showAmountWithCurrencySymbol(val?.bill?.amount_paid)}</td>
            ...
            <div className="finalcalculateiosxl44s">
                <span><p>Amount In Balance</p> <h5>{showAmountWithCurrencySymbol(payment?.amt_excess)}</h5></span>
                <span><p>Amount to be Paid</p> <h5>{showAmountWithCurrencySymbol(payment?.credit)}</h5></span>
            </div>
                ...
                <table id="tablex15s56s31s1">
                    ...
                    <tbody>
                        {payment?.entries?.map((val, index) => (
                            <tr className="rowsxs15aksx433" key={index}>
                                <td className='sfdjklsd1xs2w1'>{index + 1}</td>
                                <td className='sfdjklsd1xs2w2'>{formatDate4(val?.invoice?.transaction_date) || ""}</td>
                                <td className='sfdjklsd1xs2w3'>{val?.invoice?.invoice_id || ""}</td>
                                <td className='sfdjklsd1xs2w4 sfdjklsd1xs2wrate' style={{ width: "250px" }}>{showAmountWithCurrencySymbol(val?.invoice?.total)}</td>
                                <td className='sfdjklsd1xs2w4' >
                                    <p style={{ width: "41%", textAlign: "right" }}>
                                        {showAmountWithCurrencySymbol((+val?.invoice?.total) - (+val?.invoice?.amount_paid))}</p></td>
                                <td className='sfdjklsd1xs2w4 sfdjklsd1xs2wamount sfdjklsd1xs2wrate' style={{ width: '11%' }}>{showAmountWithCurrencySymbol(val?.amount)}</td>
                            </tr>
                        ))}
                    </tbody>
                </table> :
                    ...
                    <tbody>
                        {formData?.entries?.map((val, index) => (
                            <tr key={index} className="rowsxs15aksx433">
                                ...
                                <td className="sfdjklsd1xs2w2" style={{ paddingLeft: "40px" }}>
                                    {val?.date}
                                </td>
                                ...
                                <td className="sfdjklsd1xs2w4" style={{ textAlign: "right" }}>
                                    {showAmountWithCurrencySymbol(val?.invoice_amount?.toFixed(2))}
                                </td>
                                <td className="sfdjklsd1xs2w3" style={{ textAlign: "right" }}>
                                    {showAmountWithCurrencySymbol(val?.balance_amount?.toFixed(2))}
                                </td>
                                            ...
                                            <input
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    // Condition 2: inputValue exceeds balance amount
                                                        ...
                                                        toast('The amount entered here is more than the amount due', {
                                                            icon: '👏',
                                                            style: {
                                                                borderRadius: '10px',
                                                                background: '#333',
