...
import { binCreateAction, binDetailAction, binViewAction, rackViewAction, warehouseViewAction, zoneViewAction } from '../../../Redux/Actions/warehouseActions';
...
const CreateBins = () => {
    ...
    const handleChange = (e) => {
        ...
        let updates = { [name]: value }; // Default update

        if (name === "warehouse_id") {
            dispatch(zoneViewAction({ warehouse_id: value }));
            updates = { ...updates, zone_id: null, rack_id: null };
        } else if (name === "zone_id") {
            dispatch(rackViewAction({ zone_id: value }));
            updates = { ...updates, rack_id: null };
    ...
    const handleFormSubmit = async (e) => {
        e.preventDefault();
        try {
            dispatch(binCreateAction(formData, Navigate));
        } catch (error) {
                ...
                <div id="Anotherbox" className='formsectionx2'>
                    <div id="leftareax12">
                        <h1 id="firstheading">
                            ...
                            {isEdit ? "Update" : "New"} Bin
                        </h1><br /><br /><br /><br />

                    </div>
                    <div id="buttonsdata">
                        <Link to={"/dashboard/bin"} className="linkx3">
                            <RxCross2 />
                        </Link>
                    </div>
                </div>
                ...
                <div id="formofcreateitems" >
                    <DisableEnterSubmitForm onSubmit={handleFormSubmit}>
                        <div className="relateivdiv">
                            ...
                            <div className="itemsformwrap">
                                <div className="f1wrapofcreq">
                                    ...
                                    <div className="f1wrapofcreqx1">
                                        ...
                                        {console.log("warehouse", formData)}

                                        <div className={`form_commonblock`}
                                            onClick={!formData?.warehouse_id ? () => showToast('Please Select the Warehouse.', "error") : null}
                                        >
                                            <label className='color_red'>Zone</label>
                                            <span className={`${(!formData?.warehouse_id) && "disabledfield"}`}>
                                                {otherIcons.placeofsupply_svg}
                                                <CustomDropdown03
                                        ...
                                        <div className={`form_commonblock`}
                                            onClick={(!formData?.warehouse_id || !formData?.zone_id) ? () => showToast('Please Select the Warehouse and Zone.', "error") : null}>

                                            <label className='color_red'>Racks</label>

                                            <span className={`${(!formData?.warehouse_id || !formData?.zone_id) && "disabledfield"}`}>
                                                {otherIcons.placeofsupply_svg}
                                                <CustomDropdown03
                                    ...
                                    <div className="f1wrapofcreqx1">
                                        <div className={`form_commonblock ${!formData?.warehouse_id || !formData?.rack_id && "disabledfield"}`}>
                            ...
                            <SubmitButton2 isEdit={isEdit} itemId={itemId} cancel="bin" />
