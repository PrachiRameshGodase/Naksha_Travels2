import React from 'react'
import { otherIcons } from '../../../Helper/SVGIcons/ItemsIcons/Icons';
import { Link } from 'react-router-dom';
import { RxCross2 } from 'react-icons/rx';

const Profit_and_loss = () => {
  return (
    <>
    <div className="maindivofreporstx5w633">
    <div className="reportbarinsx5w">
        <h1>Profit and Loss</h1>
        <div className="rightfiltex4w">
            <div className="xw15sf5w5s6">{otherIcons?.print_svg}Print</div>
            <div className="xw15sf5w5s6">{otherIcons?.export_svg}Export as PDF</div>
            <Link className="crossicox45wsx2w3" to={"/dashboard/reports"}>
            <RxCross2 />
          </Link>
        </div>
      </div>

      <div className="xklwefs5d456s">
        <div className="xjlokw5456w">
            <p>Basis: Accrual</p>
            <h4>From 01 Dec 2021 To 31 Dec 2021</h4>
        </div>

        <div className="ch2xjlokw5456w">
            <div className="x45wx5w activexkl5w2">Collapse all</div>
            <div className="x45wx5w">Expand all</div>
        </div>

        
        <div className="ch2xjlokw5456wx321">
            <div className="tex5w6x6ws xkwls2z1">{otherIcons?.print_svg}ACCOUNT</div>
            <div className="tex5w6x6ws xkwls2z2">{otherIcons?.print_svg}TOTAL</div>
            <div className="tex5w6x6ws xkwls2z3">{otherIcons?.print_svg}ACCOUNT CODE</div>
        </div>


        <div className="sdf6w6xchofx32w6">
            <h3>Operating Income</h3>

            <div className="childxkwjlks5456s">
            <div className="xkw48x4s5s6x1 xkwls2z1">
                <span>General Income</span>
                <span>Interest income</span>
                <span>Late fee income</span>
                <span>Other charges</span>
                <span>Shipping charge</span>
                <span>Discount</span>
            </div>
            <div className="xkw48x4s5s6x1 xkwls2z2">
                <span>0,00</span>
                <span>0,00</span>
                <span>0,00</span>
                <span>0,00</span>
                <span>0,00</span>
                <span>0,00</span>
            </div>
            <div className="xkw48x4s5s6x1 xkwls2z3">
                <span>0,00</span>
                <span>0,00</span>
                <span>0,00</span>
                <span>0,00</span>
                <span>0,00</span>
                <span>0,00</span>
            </div>
            </div>
            
            <div className="xkwjlks5456s">
                <p className="xkwls2z1">Total for operating income</p>
                <p className="xkwls2z2">2301,76</p>
                <p className="xkwls2z3">2301,76</p>
            </div>
        </div>


        <div className="sdf6w6xchofx32w6">
            <h3>Cost of good solds</h3>

            <div className="childxkwjlks5456s">
            <div className="xkw48x4s5s6x1 xkwls2z1">
                <span>Cost of good sold</span>
                <span>Job costing</span>
                <span>Labour</span>
                <span>Materials</span>
                <span>Subcontractor</span>
            </div>
            <div className="xkw48x4s5s6x1 xkwls2z2">
                <span>0,00</span>
                <span>0,00</span>
                <span>0,00</span>
                <span>0,00</span>
                <span>0,00</span>
            </div>
            <div className="xkw48x4s5s6x1 xkwls2z3">
                <span>0,00</span>
                <span>0,00</span>
                <span>0,00</span>
                <span>0,00</span>
                <span>0,00</span>
            </div>
            </div>
            
            <div className="xkwjlks5456s">
                <p className="xkwls2z1">Total for cost of good sold</p>
                <p className="xkwls2z2">2301,76</p>
                <p className="xkwls2z3">2301,76</p>
            </div>
        </div>





        <div className="sdf6w6xchofx32w6">
            <h3>Non operating expense</h3>

            <div className="childxkwjlks5456s">
            <div className="xkw48x4s5s6x1 xkwls2z1">
                <span>Exchange gain or loss</span>
            </div>
            <div className="xkw48x4s5s6x1 xkwls2z2">
                <span>0,00</span>
            </div>
            <div className="xkw48x4s5s6x1 xkwls2z3">
                <span>0,00</span>
            </div>
            </div>
            
            <div className="xkwjlks5456s">
                <p className="xkwls2z1">Total for non operating expense</p>
                <p className="xkwls2z2">2301,76</p>
                <p className="xkwls2z3">2301,76</p>
            </div>
            <div className="xkwjlks5456s">
                <p className="xkwls2z1">Net  profit/loss</p>
                <p className="xkwls2z2">2301,76</p>
                <p className="xkwls2z3">2301,76</p>
            </div>
        </div>



      </div>
    </div>
    </>
  )
}

export default Profit_and_loss
