...
import { PassengerAssistDeleteActions } from "../../../../Redux/Actions/passengerAssistActions";
...
const Assit = ({ data, totalItems }) => {
  ...
  const handleDeleteAssist = async (item) => {
    ...
    if (result.isConfirmed) {
      const sendData = {
        dsr_assists_id: item?.id,
      };
      dispatch(PassengerAssistDeleteActions(sendData)).then((response) => {
        if (itemId) {
          const refreshData = {
            passenger_id: itemId,
          };
