    ...
    const handleSelect = (option) => {
        if (option.active !== "0") {
            ...
            onChange({
                target: {
                    name,
                    value: type === "masters" ? option.labelid : type === "service" ? option?.label : type === "masters2" ? option?.label : type === "item_type" ? option?.label : type === "taxRate" ? option?.tax_percentge : type === "currency" ? option?.code : type === "rate" ? option : type === "masters_salutation" ? option?.label : type === 'countryList' ? option?.country_id : type == "visa_entry_type" ? option?.visa_entry_type : type === "visa_type_id" ? option.visa_type_id : type === "days" ? option.days : type === "airportList" ? option?.name : type === "airportList2" ? option?.airport : type == "package_name"? option?.package_name : type === "meetingType" ? option?.meeting_type : type === "noOfPersons" ? option?.no_of_person : type === "select_item2" ? option?.flight_name : type === "companyList" ? option?.company_name : type === "account" ? option?.account_type : option.id,
                }
            });
            ...
            if (type === "vendor" || type == "purchase" || type == "countryList" || type == "visa_entry_type" || type == 'visa_type_id' || type == "days" || type == "airportList" || type == "hotelList" || type == "meetingType" || type == "noOfPersons" || type == "airportList2" || type == "companyList" ) {
                setcusData(option);
            } else if (type === "select_item" || type == "package_name") {
                setItemData(option)
            }
