...
import { activeOrg_details, currencySymbol, parseJSONofString, sendData, ShowMasterData } from '../HelperFunctions';
...
const ItemSelect = ({ formData, setFormData, handleChange, setIsItemSelect, isItemSelect, extracssclassforscjkls, dropdownRef2, note, shwoCharges, invoice_section, section }) => {
    ...
    console.log("isIdEqualState", isIdEqualState?.isId)
    ...
    useEffect(() => {
        ...
        const tax_amount = newItems?.reduce((acc, item) => acc + parseFloat(item?.tax_amount || 0), 0);
        const gross_amount = newItems?.reduce((acc, item) => acc + parseFloat(item?.gross_amount || 0), 0);
        ...
        newItems.forEach((item, index) => {
            if (item?.tax_name === "Taxable" && item?.tax_rate) {
                ...
                if (isIdEqualState?.isId) {
                    ...
                    const existingTax = taxes.find(tax => tax.CGST === cgstRate && tax.SGST === sgstRate);
                    ...
                    // Check if the tax group already exists
                    const existingTax = taxes.find(tax => tax.IGST === applicableTaxRate);
    ...
    const handleItemChange = (index, field, value) => {
        ...
        const discount =
            item?.discount_type == 1 ? discountAmount : (grossAmount * discountPercentage) / 100;
                            ...
                            <div className='table_item_border'>
                                ...
                                {item?.hsn_code &&
                                    <div className="itemDetailsData_900">
                                        ...
                                        <p>HSN Code: <span className='item_hsn_code_9'>{item?.hsn_code}</span></p>
                                    </div>
                                }
                ...
                <div className='secondtotalsections485s'>
                            ...
                            <div className='textareaofcreatqsiform'>
                                ...
                                <div className='show_no_of_text_limit_0121'>
                                    <TextAreaComponentWithTextLimit
                                        ...
                                        name="customer_note"
                                        value={formData?.customer_note == 0 ? "" : formData?.customer_note}
                                    />
                                </div>
                            </div>
                            :
                    ...
                    <div className="calctotalsection">
                        ...
                        <div className="calcuparentc">
                            ...
                            {/* Add expense changes */}
                            <div className='clcsecx12s1'>
                                <label><p className='edit_changes_021' onClick={openExpenseCharges}> Edit and add charges</p></label>
                            </div>
                            {openCharges && <ExpenseCharges formValues={{ formData, setFormData, handleChange }} />}
...
export const ItemSelectGRM = ({ formData, setFormData, handleChange, setIsItemSelect, isItemSelect, extracssclassforscjkls, dropdownRef2, note, itemData1, imgLoader,
    setImgeLoader, vendorList, setIsGrnQntySelect }) => {
    const itemList = useSelector((state) => state?.itemList);
    const dropdownRef = useRef(null);
    const buttonRef = useRef(null);
    ...
    useEffect(() => {
        ...
        const total_charges_type = newChargesType?.reduce((total, item) => total + (+item?.amount || 0), 0);
    ...
    }, [formData?.items, formData?.total_charges, formData?.charges, formData?.total, formData?.charges_type]);
    //set all the values of items// when do any changes in items
    ...
    const handleItemChange = (index, field, value) => {
        ...
        const updatedItems = newItems.map((item, i) => {
            if (i === index || field === "amount" || field === "custom_duty") {
                ...
                const chargesWeight = updatedTotalCharges ? (grossAmount / updatedTotalCharges) : 0;
        ...
        const tax_amount = updatedItems?.reduce((acc, item) => acc + parseFloat(item?.tax_amount), 0);
        const subtotal = updatedItems.reduce((acc, item) => acc + parseFloat(item?.final_amount || 0), 0);
                ...
                <>
                    <div className='itemsectionrows1' style={{ minWidth: "1225px" }}>
                        <div className="tableheadertopsxs1" id='tableheadertopsxs1'>
                            ...
                            <p className='tablsxs1a3x3 tablsxs1a2x3'>GRN QTY<b className="color_red">*</b></p>
                                    ...
                                    <div className="tablsxs1a5x3" id='ITEM_Selection6'>
                                        <span >
                                            <CustomDropdown04
                                                options={unitList}
                                                value={item?.unit_id}
                                                onChange={(e) => handleItemChange(index, 'unit_id', e.target.value, e.target.option)}
                                                name="unit_id"
                                                defaultOption="Units"
                                                type="masters"
